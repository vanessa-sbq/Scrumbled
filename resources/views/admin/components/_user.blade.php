    ...
    <tr>
        ...
        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 hidden md:table-cell">
            <div class="flex space-x-2">
                <form method="POST" action="{{$user->status === "BANNED" ? route('admin.users.unban', $user->id) : route('admin.users.ban', $user->id)}}">
                    @csrf
                    @method('POST')
                ...
                <a href="{{ route('admin.users.edit', $user->username) }}" class="bg-yellow-500 text-white px-3 py-1 rounded-md hover:bg-yellow-700 transition">
                    Edit
                </a>
            </div>
        </td>
        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 hidden md:table-cell">
            <a href="{{ route('admin.users.show', $user->username) }}" target="_blank" rel="noopener" class="bg-primary text-white px-4 py-2 rounded-md shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                View Profile
            </a>
        </td>
    </tr>
