...
@section('content')
    <section class="container pt-6 max-w-lg">
        ...
        <form class="bg-white rounded-card shadow-md flex flex-col gap-6 p-8"
            action="{{ route('admin.users.edit', $user->username) }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex flex-col gap-4">
                <div class="space-y-2 place">
                    ...
                    <img class="h-24 w-24 rounded mx-auto" id="old_picture" src="{{ asset($imagePath) }}"
                        alt="Profile Picture">
                </div>
                <div>
            ...
            <div>
                ...
                <input type="text" id="username" name="username" placeholder="Enter a unique username"
                    class="w-full px-3 py-2 border rounded" value="{{ $user->username }}" required>
                @error('username')
                    <div class="text-red-500 mt-2">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="email" id="email" name="email" placeholder="Enter a valid email address"
                    class="w-full px-3 py-2 border rounded" value="{{ $user->email }}" required>
                @error('email')
                    <div class="text-red-500 mt-2">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" id="full_name" name="full_name" placeholder="Enter the user's full name"
                    class="w-full px-3 py-2 border rounded" value="{{ $user->full_name }}" required>
                @error('full_name')
                    <div class="text-red-500 mt-2">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="bio" name="bio" placeholder="Write a short biography" class="w-full px-3 py-2 border rounded">{{ $user->bio }}</textarea>
                @error('bio')
                    <div class="text-red-500 mt-2">{{ $message }}</div>
                @enderror
            </div>
