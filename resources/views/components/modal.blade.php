<div id="{{ $attributes->get('id') }}"
    ...
    <div class="w-full max-w-lg bg-white shadow-lg rounded-lg p-6 relative">
        ...
        <div class="border-t border-gray-300 pt-6 flex justify-end gap-4">
            ...
            <button onclick="saveModal('{{ $attributes->get('id') }}')" type="button"
                class="px-4 py-2 rounded-lg text-white text-sm border-none outline-none tracking-wide bg-blue-600 hover:bg-blue-700 active:bg-blue-600">{{ $saveButtonText }}</button>
        </div>
    </div>
</div>

<div onclick="openModal('{{ $attributes->get('id') }}')">
    {{ $trigger }}
</div>
