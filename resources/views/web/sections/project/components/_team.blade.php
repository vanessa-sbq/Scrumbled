<div class="flex flex-col bg-white p-6 rounded-lg shadow-md">
    ...
    @if ($developers->isNotEmpty())
        <p class="space-y-4">
            ...
            @endforeach
            @if (Auth::guard('admin')->check() || auth()->id() === $project->product_owner_id)
                <x-modal id="remove_developer_modal" title="Developer removal" closeButtonText="Cancel"
                    saveButtonText="Yes, remove" saveAction="removeDeveloperHelper" activeButtonColor="bg-red-600"
                    hoverButtonColor="bg-red-700">
                    <div>
