...
@section('content')
    <div class="container mx-auto py-8">
        ...
        <form method="POST" action="{{ route('tasks.createNew', ['slug' => $slug]) }}" class="max-w-lg mx-auto bg-white p-8 rounded-card shadow-md">
            @csrf
            
            <!-- Title -->
            <div class="mb-4">
            <label for="title" class="block text-sm font-medium text-muted-foreground">Title</label>
            <input id="title" type="text" name="title" value="{{ old('title') }}" required autofocus class="mt-1 block w-full px-3 py-2 border border-muted rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
            @error('title')
            ...
            <textarea id="description" name="description" class="mt-1 block w-full px-3 py-2 border border-muted rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">{{ old('description') }}</textarea>
            @error('description')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
            </div>
            ...
            <select id="value" name="value" required class="mt-1 block w-full px-3 py-2 border border-muted rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
                ...
                <option value="WILL_NOT_HAVE" {{ old('value') == 'WILL_NOT_HAVE' ? 'selected' : '' }}>Will Not Have</option>
            </select>
            @error('value')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
