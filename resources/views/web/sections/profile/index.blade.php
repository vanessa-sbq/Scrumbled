                ...
                @foreach ($users as $user)
                    <li class="bg-white shadow-md rounded-card p-6 mb-5">
                        <div class="flex items-center space-x-4">
                            <img src="{{ asset('storage/' . $user->picture) }}" alt="{{ $user->full_name }}'s profile picture" class="w-16 h-16 rounded">
                            <div>
                                <a href="{{ route('show.profile', $user->username) }}" class="font-bold hover:underline">{{ $user->username }}</a><br>
                                <a href="{{ route('show.profile', $user->username) }}" class="hover:underline">{{ $user->full_name }}</a> (<a>{{ $user->email }}</a>)
                            </div>
                        </div>
                    </li>
                @endforeach
