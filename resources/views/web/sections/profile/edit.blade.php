...
@section('content')
    ...
    <div class="max-w-lg container pt-6">
        ...
        <form class="bg-white rounded-card shadow-md flex flex-col gap-6 p-8" action="{{ route('edit.profile', $user->username) }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex flex-col gap-4">
                <div class="space-y-2 place">
                    ...
                    <img class="h-24 w-24" id="old_picture" src="{{asset('storage/' . $user->picture)}}" alt="Profile Picture">
                </div>
                <div>
            ...
            <a class="px-4 py-2 bg-red-600 text-white hover:bg-red-800 rounded text-center" href="{{route('show.profile', $user->username)}}">Cancel</a>
        </form>
    </div>
@endsection
