...
@section('content')
    <div class="container mx-auto py-8">
        ...
        <form action="{{ route('tasks.editTask', ['slug' => $slug, 'id' => $task->id]) }}" method="POST"
            class="max-w-lg mx-auto card">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select id="value" name="value" required
                    ...
                    <option value="MUST_HAVE" {{ old('value', $task->value) == 'MUST_HAVE' ? 'selected' : '' }}>Must Have
                    </option>
                    <option value="SHOULD_HAVE" {{ old('value', $task->value) == 'SHOULD_HAVE' ? 'selected' : '' }}>Should
                        Have</option>
                    <option value="COULD_HAVE" {{ old('value', $task->value) == 'COULD_HAVE' ? 'selected' : '' }}>Could Have
                    </option>
                    <option value="WILL_NOT_HAVE" {{ old('value', $task->value) == 'WILL_NOT_HAVE' ? 'selected' : '' }}>Will
                        Not Have</option>
                </select>
                @error('value')
