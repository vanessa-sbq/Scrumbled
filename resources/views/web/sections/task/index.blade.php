...
@section('content')
    <div class="container p-4 md:py-16">
        ...
        <form method="GET" action="{{ route('projects.tasks.search', ['slug' => $project->slug]) }}" class="flex gap-2">
            <input type="text" name="query" id="task-search-input" class="w-full px-3 py-2 mb-4 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" placeholder="Search tasks...">
            <button type="submit" class="px-3 py-2 mb-4 bg-primary text-white rounded-md hover:bg-primary-dark">Search</button>
        </form>
